...
// Assets - Replace these with your actual Vintage Sedan image paths
import vintageMain from "../images/Vintage-Classic-Car.jpg"; 
import vintageInterior from "../images/Vintage-Classic-Car-front.jpg";
import vintageSideView from "../images/Vintage-Classic-Car-back.jpg";

const masterFleetData = [
  { 
    id: 701, 
    path: "vintage-classic", 
    type: "Vintage Sedan", 
    models: "Vintage Classic Car", 
    seats: "5 SEATS", 
    bags: "3 BAGS", 
    pricePerKm: 120, // Premium pricing for heritage fleet
    rating: 5.0, // Based on source card
    isAc: true, 
    img: vintageMain,
    description: "Step back in time with the Vintage Classic Car, the epitome of heritage luxury and timeless design. This meticulously restored vintage sedan offers a unique travel experience that blends the grandeur of a bygone era with modern reliability. Featuring plush leather interiors, polished chrome accents, and an iconic silhouette, it is the premier choice for weddings, high-profile events, and luxury heritage tours across the city."
  }
];
...
const VintageDetails = () => {
  ...
  if (!carData) {
    return <div className="min-h-screen bg-black flex items-center justify-center font-bold text-white uppercase tracking-tighter">Preparing Heritage Fleet...</div>;
  }

  const details = {
    brand: "VINTAGE SEDAN",
    model: carData.models.toUpperCase(),
    category: "HERITAGE LUXURY COLLECTION",
    driveType: "CHAUFFEUR DRIVEN",
    description: carData.description,
    images: [carData.img, vintageInterior, vintageSideView],
    features: [ `${carData.seats}`, "HERITAGE INTERIOR", "AIR CONDITIONED", `${carData.bags}` ]
  };

  const packagesData = [
    { id: 'city', title: 'HERITAGE CITY TOUR — 8HR 80KM', basePrice: 25000, extraKmRate: 120, extraHrRate: 2500, hasExtras: true },
    { id: 'wedding', title: 'WEDDING SPECIAL — 4HR 40KM', basePrice: 15000, extraKmRate: 150, extraHrRate: 3000, hasExtras: true },
    { id: 'airport', title: 'VIP HERITAGE TRANSFER', basePrice: 8500, extraKmRate: 0, extraHrRate: 0, hasExtras: false }
  ];
          ...
          <div className="flex items-center gap-4 md:gap-5">
            <div className="flex items-center text-white">
                <div className="w-12 h-12 md:w-14 md:h-14 rounded-full border-[1.5px] border-white flex items-center justify-center">
                  <span className="font-bold text-[9px] md:text-[10px] tracking-tighter text-white text-center">VNTG</span>
                </div>
            </div>
                ...
                <div className={`px-6 md:px-12 overflow-hidden transition-all duration-500 ${isActive ? 'max-h-[800px] pb-8 opacity-100' : 'max-h-0 opacity-0'}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-6 mt-4 bg-slate-50 p-6 rounded-2xl border border-slate-200">
                    <div className="flex flex-col">
                      <label className="text-slate-600 text-[10px] md:text-xs font-bold uppercase tracking-widest mb-2">Heritage Date *</label>
                    ...
                    <div className="flex flex-col">
                      <label className="text-slate-600 text-[10px] md:text-xs font-bold uppercase tracking-widest mb-2">Arrival Time *</label>
