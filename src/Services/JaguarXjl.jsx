...
// Placeholders: Aap apni actual Jaguar XJL images yahan import karein
// import jaguarxjlview from "../images/jaguarsjl.png"; // Replace with Jaguar XJL image
import jaguarxjlview from "../images/jaguarxjlview.png"
import jaguarxjlfrontview from "../images/jaguarxjlfrontview.png"; // Replace with Jaguar XJL image
import jaguarxjlbackview from "../images/jaguarxjlbackview.png"; // Replace with Jaguar XJL image

// Master Data specifically configured for Jaguar XJL (Flagship LWB)
const masterFleetData = [
  { 
    ...
    type: "Jaguar", 
    models: "JAGUAR XJL", 
    seats: "4 SEATS", // 4-seater to emphasize the VIP rear lounge experience
    bags: "3 BAGS", 
    pricePerKm: 75, // Flagship luxury sedan pricing
    rating: 5.0, 
    isAc: true, 
    img: jaguarxjlview,
    description: "The Jaguar XJL is the pinnacle of British luxury and flagship elegance. As the long-wheelbase version of the iconic XJ, it offers an exceptionally spacious rear cabin designed for the ultimate VIP experience. Featuring quilted leather seating, a panoramic glass roof, and whisper-quiet acoustic insulation, the XJL is a true sanctuary on wheels. With its distinctive sweeping silhouette and dynamic performance, it is the perfect choice for high-profile corporate transfers, royal weddings, and elite outstation travel."
  }
];
...
const CabDetails = () => {
  ...
  useEffect(() => {
    const foundCar = masterFleetData.find(car => car.path === carName) || masterFleetData.find(car => car.id === 108);
    setCarData(foundCar);
  }, [carName]);
  ...
  // Structuring data for Jaguar XJL
  const details = {
    brand: carData.type.toUpperCase(),
    model: carData.models.toUpperCase(),
    category: "ULTRA LUXURY SEDAN", // Flagship Category
  ...
  // Mock Packages Data (Flagship Pricing)
  const packagesData = [
    {
      id: 'city',
        ...
        <div className="flex flex-row items-center justify-between mb-8 px-1 md:px-2">
          <div className="flex items-center gap-4 md:gap-5">
            <div className="flex items-center text-white">
                {/* Sleek Minimalist Jaguar Logo Representation */}
                <div className="w-12 h-12 md:w-14 md:h-14 rounded-full border-[1.5px] border-white flex items-center justify-center p-1.5 shadow-[0_0_15px_rgba(255,255,255,0.1)]">
                  <span className="font-serif font-bold text-xl md:text-2xl tracking-widest text-white">J</span>
                </div>
            </div>
