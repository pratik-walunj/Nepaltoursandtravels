...
import urbania17Main from "../images/urbania17.jpg"; 
import urbania17Interior from "../images/urbania17Front.jpg";
import urbania17Rear from "../images/urbania17Back.jpg";
...
const masterFleetData = [
  { 
    ...
    path: "urbania-17", 
    type: "Force Motors", 
    models: "17 Seater Luxury Urbania", 
    seats: "17 SEATS", 
    bags: "14 BAGS", 
    pricePerKm: 55, 
    rating: 4.9, 
    isAc: true, 
    img: urbania17Main,
    description: "The 17 Seater Force Urbania is the definitive choice for large-group luxury travel. With its high-roof design, wide aisles, and plush reclining seats, it offers an unparalleled travel experience. Equipped with individual AC vents, reading lights, and massive luggage capacity, it is perfectly suited for corporate tours, pilgrimage groups, and extended family vacations across Nepal."
  }
];

const Urbania17Details = () => {
  ...
  if (!carData) {
    return <div className="min-h-screen bg-black flex items-center justify-center font-bold text-white uppercase tracking-tighter">Initializing Elite Transporter...</div>;
  }

  const details = {
    brand: "FORCE URBANIA",
    model: carData.models.toUpperCase(),
    category: "ELITE GROUP TRANSPORTER",
    driveType: "CHAUFFEUR DRIVEN",
    description: carData.description,
    images: [carData.img, urbania17Interior, urbania17Rear],
    features: [ `${carData.seats}`, "DUAL COMPRESSOR AC", "RECLINING SEATS", `${carData.bags}` ]
  };

  const packagesData = [
    { id: 'city', title: 'ELITE CITY — 8HR 80KM', basePrice: 10500, extraKmRate: 55, extraHrRate: 1000, hasExtras: true },
    { id: 'outstation', title: 'TOUR EXPEDITION — MIN 300KM / DAY', basePrice: 18000, extraKmRate: 60, extraHrRate: 0, hasExtras: true },
    { id: 'airport', title: 'AIRPORT GROUP VIP TRANSFER', basePrice: 5000, extraKmRate: 0, extraHrRate: 0, hasExtras: false }
  ];
  ...
  return (
    <div className="font-sans bg-black min-h-screen pt-24 md:pt-32 pb-20 px-4 sm:px-6 lg:px-8 relative selection:bg-gray-700 selection:text-white">
      ...
      {/* --- 1. MAIN VEHICLE CARD (Standard Dark Theme) --- */}
      <div className="w-full max-w-[1000px] mx-auto bg-[#181818] rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-10 shadow-2xl mb-8 border border-white/5">
        ...
        {/* --- IMAGE CONTAINER (Light Gray Studio Pop Style) --- */}
      ...
      {/* --- PACKAGE SELECTION (Standard White Accordion) --- */}
              ...
              <div key={pkg.id} className={`border-b border-slate-200 ${index === packagesData.length - 1 ? 'border-b-0' : ''}`}>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-3.5 h-3.5 rounded-full border-2 transition-colors ${isActive ? 'bg-black border-black' : 'border-slate-300'}`}>
                ...
                <div className={`px-6 md:px-12 overflow-hidden transition-all duration-500 ${isActive ? 'max-h-[800px] pb-8 opacity-100' : 'max-h-0 opacity-0'}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-6 mt-4 bg-slate-50 p-6 rounded-2xl border border-slate-200 shadow-inner">
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-5 mt-4 bg-[#181818] p-6 md:p-8 rounded-2xl shadow-xl border border-black">
                    <div>
                      <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-1">Elite Journey Total</p>
                      <p className="text-3xl md:text-4xl font-black text-white">₹ {currentTotal.toLocaleString('en-IN')}</p>
                      <p className="text-gray-500 text-[10px] mt-1 font-medium">State taxes & tolls extra as per actuals</p>
                    </div>
                    <button 
...
export default Urbania17Details;