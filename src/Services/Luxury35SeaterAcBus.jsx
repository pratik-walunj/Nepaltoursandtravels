...
// Placeholders: Aap apni actual 35-Seater Bus images yahan import karein
import acbus35seaterview from "../images/acbus35seaterview.png"; // Replace with 35-Seater Bus image
import acbus35seaterfrontview from "../images/acbus35seaterfrontview.png"; // Replace with 35-Seater Bus image
import acbus35seaterbackview from "../images/acbus35seaterbackview.png"; // Replace with 35-Seater Bus image

// Master Data specifically configured for Luxury 35-Seater Bus
const masterFleetData = [
  { 
    id: 206, 
    path: "luxury-35-seater-ac-bus", 
    type: "Bus", 
    models: "35 SEATER LUXURY BUS", 
    seats: "35 SEATS", 
    bags: "35 BAGS", // Ample underbelly luggage capacity for 35 passengers
    pricePerKm: 65, // Commercial mid-size coach pricing
    rating: 4.8, 
    isAc: true, 
    img: acbus35seaterview,
    description: "The 35-Seater Luxury AC Bus provides the perfect balance of scale and premium comfort for mid-sized group travel. Ideal for destination weddings, corporate retreats, and extended family vacations, this modern coach features deeply contoured pushback seating, individual AC climate control, and a smooth air-suspension system. With ample luggage capacity and a beautifully appointed interior, your group can enjoy a relaxing, first-class journey together without the hassle of coordinating multiple cars."
  }
];
...
const CabDetails = () => {
  ...
  useEffect(() => {
    const foundCar = masterFleetData.find(car => car.path === carName) || masterFleetData.find(car => car.id === 206);
    setCarData(foundCar);
  }, [carName]);
  ...
  // Structuring data for 35-Seater Bus
  const details = {
    brand: carData.type.toUpperCase(),
    model: carData.models.toUpperCase(),
    category: "MID-SIZE LUXURY COACH", 
    driveType: "CHAUFFEUR DRIVEN",
    description: carData.description,
    images: [
      carData.img, 
      acbus35seaterfrontview, 
      acbus35seaterbackview
    ],
    features: [
      `${carData.seats}`,
  ...
  // Mock Packages Data (Commercial Mid-Size Coach Pricing)
  const packagesData = [
    {
      id: 'city',
  ...
  return (
    <div className="font-sans bg-black min-h-screen pt-24 md:pt-32 pb-20 px-4 sm:px-6 lg:px-8 relative selection:bg-gray-700 selection:text-white">
      ...
      <div className="w-full max-w-[1000px] mx-auto bg-[#181818] rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-10 shadow-2xl mb-8 border border-white/5">
        ...
        <button 
          onClick={() => navigate(-1)} 
          className="text-gray-400  cursor-pointer hover:text-white font-bold transition-all text-[10px] md:text-xs uppercase tracking-widest flex items-center mb-6"
        >
