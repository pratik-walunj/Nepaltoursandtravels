...
const masterFleetData = [
  { 
    id:199, 
    path: "12-seater-tempo-traveller", 
    type: "Force", 
    models: "12 SEATER TEMPO TRAVELLER", 
    seats: "12 SEATS", 
    bags: "8 BAGS", // Perfect luggage capacity for 12 passengers
...
const CabDetails = () => {
  ...
  useEffect(() => {
    const foundCar = masterFleetData.find(car => car.path === carName) || masterFleetData.find(car => car.id === 199);
    setCarData(foundCar);
  }, [carName]);
  ...
  // Structuring data for 12-Seater Tempo Traveller
  const details = {
    brand: carData.type.toUpperCase(),
    model: carData.models.toUpperCase(),
    category: "LUXURY COLLECTION", // Premium vibe
  ...
  // Mock Packages Data (Pricing adjusted for 12-Seater)
  const packagesData = [
    {
      id: 'city',
  ...
  return (
    <div className="font-sans bg-black min-h-screen pt-24 md:pt-32 pb-20 px-4 sm:px-6 lg:px-8 relative selection:bg-gray-700 selection:text-white">
      ...
      {/* GLOBAL BACK BUTTON (Placed outside the card like standard websites) */}
      {/* <button 
        onClick={() => navigate(-1)} 
