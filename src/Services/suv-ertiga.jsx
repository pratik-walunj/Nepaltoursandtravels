...
// Assets - Replace with your actual Ertiga image paths
...
const ErtigaDetails = () => {
  ...
  if (!carData) {
    return <div className="min-h-screen bg-black flex items-center justify-center font-bold text-white uppercase tracking-tighter">Initializing MPV...</div>;
  }

  const details = {
    brand: carData.type.toUpperCase(),
    model: carData.models.toUpperCase(),
    category: "PREMIUM MPV COLLECTION",
    driveType: "CHAUFFEUR DRIVEN",
    description: carData.description,
    images: [carData.img, ertigaFront, ertigaInterior],
    features: [ `${carData.seats}`, "AUTOMATIC AC", "PETROL/CNG", `${carData.bags}` ]
  };

  const packagesData = [
    { id: 'city', title: 'LOCAL RIDE — 8HR 80KM', basePrice: 3500, extraKmRate: 18, extraHrRate: 250, hasExtras: true },
    { id: 'outstation', title: 'OUTSTATION — MIN 300KM / DAY', basePrice: 5400, extraKmRate: 20, extraHrRate: 0, hasExtras: true },
    { id: 'airport', title: 'AIRPORT DROP/PICKUP', basePrice: 1500, extraKmRate: 0, extraHrRate: 0, hasExtras: false }
  ];
  ...
  return (
    <div className="font-sans bg-black min-h-screen pt-24 md:pt-32 pb-20 px-4 sm:px-6 lg:px-8 relative selection:bg-gray-700 selection:text-white">
      ...
      {/* --- 1. MAIN VEHICLE CARD (High-Contrast Theme) --- */}
          ...
          <div className="flex items-center gap-4 md:gap-5">
            <div className="flex items-center text-white">
                <div className="w-12 h-12 md:w-14 md:h-14 rounded-full border-[1.5px] border-white flex items-center justify-center">
                  <span className="font-bold text-[10px] md:text-xs tracking-tighter text-white">MPV</span>
                </div>
            </div>
