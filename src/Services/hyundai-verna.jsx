...
// Assets - Replace with your actual Hyundai Verna image paths
import vernaMain from "../images/verna.jpg"; 
import vernaFront from "../images/verna_front.jpg";
import vernaInterior from "../images/verna_back.jpg";
...
const masterFleetData = [
  { 
    ...
    isAc: true, 
    img: vernaMain,
    description: "The Hyundai Verna is the epitome of sophisticated performance. Featuring a sleek fastback silhouette and a tech-laden interior, it is designed for those who appreciate the finer things in travel. With its smooth suspension and whisper-quiet cabin, the Verna is perfect for executive city transfers and comfortable long-distance highway cruising."
  }
];
...
const VernaDetails = () => {
  ...
  if (!carData) {
    return <div className="min-h-screen bg-black flex items-center justify-center font-bold text-white uppercase tracking-tighter">Initializing Executive Sedan...</div>;
  }
  ...
  const details = {
    ...
    images: [carData.img, vernaFront, vernaInterior],
    features: [ `${carData.seats}`, "BOSE SOUND", "VENTILATED SEATS", `${carData.bags}` ]
  };

  const packagesData = [
    { id: 'city', title: 'LOCAL RIDE — 8HR 80KM', basePrice: 3800, extraKmRate: 18, extraHrRate: 250, hasExtras: true },
    { id: 'outstation', title: 'OUTSTATION — MIN 300KM / DAY', basePrice: 5500, extraKmRate: 20, extraHrRate: 0, hasExtras: true },
    { id: 'airport', title: 'AIRPORT EXECUTIVE TRANSFER', basePrice: 1500, extraKmRate: 0, extraHrRate: 0, hasExtras: false }
  ];
      ...
      <div className="w-full max-w-[1000px] mx-auto bg-[#181818] rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-10 shadow-2xl mb-8 border border-white/5">
        ...
        <div className="flex flex-row items-center justify-between mb-8 px-1 md:px-2">
          <div className="flex items-center gap-4 md:gap-5">
            <div className="flex items-center text-white">
                <div className="w-12 h-12 md:w-14 md:h-14 rounded-full border-[1.5px] border-white flex items-center justify-center">
                  <span className="font-bold text-[10px] md:text-xs tracking-tighter text-white">SEDAN</span>
                </div>
            </div>
          ...
          <div className="flex flex-col items-end">
            {/* <span className="text-indigo-400 text-[10px] md:text-xs font-black uppercase tracking-[0.1em] bg-indigo-500/10 px-3 py-1 rounded-md mb-1">
              ...
              <div key={pkg.id} className={`border-b border-slate-200 ${index === packagesData.length - 1 ? 'border-b-0' : ''}`}>
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-3.5 h-3.5 rounded-full border-2 transition-colors ${isActive ? 'bg-indigo-600 border-indigo-600' : 'border-slate-300'}`}>
                ...
                <div className={`px-6 md:px-12 overflow-hidden transition-all duration-500 ${isActive ? 'max-h-[800px] pb-8 opacity-100' : 'max-h-0 opacity-0'}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-6 mt-4 bg-slate-50 p-6 rounded-2xl border border-slate-200">
                    <div className="flex flex-col">
                      <label className="text-slate-600 text-[10px] md:text-xs font-bold uppercase tracking-widest mb-2">Schedule Date *</label>
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-5 mt-4 bg-[#181818] p-6 md:p-8 rounded-2xl shadow-lg border border-black">
                    <div>
                      <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-1">Total Estimated Fare</p>
