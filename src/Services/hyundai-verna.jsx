...
// Replace with your actual Hyundai Verna image paths
...
const masterFleetData = [
  { 
    ...
    isAc: true, 
    img: vernaMain,
    description: "The Hyundai Verna is the epitome of sophisticated performance. Featuring a sleek fastback silhouette and an tech-laden interior, it is designed for those who appreciate the finer things in travel. With its smooth suspension and whisper-quiet cabin, the Verna is perfect for executive city transfers and comfortable long-distance highway cruising. Experience the 'Futuristic Ferocity' of Nepal's favorite premium sedan."
  }
];
...
const VernaDetails = () => {
  ...
  const details = {
    ...
    features: [carData.seats, "VENTILATED SEATS", "BOSE SOUND SYSTEM", carData.bags]
  };
    ...
    <div className="font-sans bg-black min-h-screen py-10 px-4 sm:px-6 lg:px-8 relative selection:bg-gray-800 pb-20">
      ...
      <div className="w-full max-w-5xl mx-auto bg-[#1c1c1c] rounded-[2.5rem] p-6 md:p-8 lg:p-12 shadow-2xl mb-12 mt-8">
        ...
        <div className="border-t border-gray-800 pt-8">
          <h3 className="text-gray-500 text-xs font-bold uppercase tracking-widest mb-4">The New Benchmark of Luxury</h3>
      ...
      <div className="w-full max-w-5xl mx-auto mt-16">
        <h2 className="text-lg md:text-xl font-extrabold text-white tracking-widest uppercase mb-6 pl-2">Executive Verna Packages</h2>
              ...
              <div key={pkg.id} className={`border-b border-black/50 ${idx === packagesData.length - 1 ? 'border-b-0' : ''}`}>
                <div className="flex items-center justify-between p-6 md:p-8 cursor-pointer hover:bg-white/5" onClick={() => { setActivePackage(isActive ? null : pkg.id); setExtraKms(''); setExtraHrs(''); }}>
                  <div className="flex items-center gap-4">
                    <div className={`w-3.5 h-3.5 rounded-full ${isActive ? 'bg-indigo-500' : 'border border-gray-500'}`} />
                ...
                <div className={`px-6 md:px-14 overflow-hidden transition-all duration-500 ${isActive ? 'max-h-[800px] pb-8 opacity-100' : 'max-h-0 opacity-0'}`}>
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-end justify-between border-t border-gray-800 pt-8">
                    <div>
                      <p className="text-gray-500 text-[10px] font-bold uppercase mb-1">Executive Travel Fare</p>
                      <p className="text-3xl font-black text-white">₹ {currentTotal.toLocaleString('en-IN')}</p>
                    </div>
                    <button className="w-full md:w-auto bg-white text-black font-black uppercase text-xs py-4 px-12 rounded-full mt-6 md:mt-0 active:scale-95 transition-transform">
                      Reserve Executive Verna
                    </button>
                  </div>
                </div>
              </div>
