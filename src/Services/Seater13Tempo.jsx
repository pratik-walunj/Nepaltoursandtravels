...
// Placeholders: Aap apni actual 13-Seater Tempo Traveller images yahan import karein
import seater13view from "../images/seater13view.png"; 
import seater13frontview from "../images/seater13frontview.png"; 
import seater13backview from "../images/seater13backview.png"; 

// Master Data specifically configured for 13-Seater Tempo Traveller
const masterFleetData = [
  { 
    id: 200, 
    path: "13-seater-tempo-traveller", 
    type: "Force", 
    models: "13 SEATER TEMPO TRAVELLER", 
    seats: "13 SEATS", 
    bags: "9 BAGS", 
    pricePerKm: 24, 
    rating: 4.8, 
    isAc: true, 
    img: seater13view,
    description: "The 13-Seater Luxury Tempo Traveller is the ideal choice for medium-sized groups, bridging the gap between a standard SUV and a large commercial van. Perfect for weekend getaways, mid-sized family trips, and corporate airport transfers, it offers a highly refined and comfortable cabin. Passengers can enjoy premium pushback reclining seats, dedicated AC vents for uniform cooling, and a high-quality music system. With ample luggage space and a smooth suspension setup, it ensures a fatigue-free and memorable journey for your entire group."
  }
];
...
const CabDetails = () => {
  ...
  useEffect(() => {
    const foundCar = masterFleetData.find(car => car.path === carName) || masterFleetData.find(car => car.id === 200);
    setCarData(foundCar);
  }, [carName]);
  ...
  // Structuring data for 13-Seater Tempo Traveller
  const details = {
    brand: carData.type.toUpperCase(),
    model: carData.models.toUpperCase(),
    category: "LUXURY COLLECTION", // Updated to match the screenshot vibe
  ...
  return (
    <div className="font-sans bg-black min-h-screen pt-24 md:pt-32 pb-20 px-4 sm:px-6 lg:px-8 relative selection:bg-gray-700 selection:text-white">
      ...
      {/* --- 1. MAIN CAR DETAILS CARD (EXACT SCREENSHOT MATCH) --- */}
      <div className="w-full max-w-[1000px] mx-auto bg-[#181818] rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-10 shadow-2xl mb-8 border border-white/5">
        ...
        {/* Back Button (Inside Card, Above Logo) */}
        <button 
          onClick={() => navigate(-1)} 
        ...
        <div className="flex flex-row items-center justify-between mb-8 px-1 md:px-2">
          ...
          <div className="hidden md:block">
            {/* Raw text just like the screenshot, no pill background */}
