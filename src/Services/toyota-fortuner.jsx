...
// Assets - Replace with your actual Fortuner image paths
import fortunerMain from "../images/fortuner.jpg"; 
import fortunerFront from "../images/fortuner_front.jpg";
import fortunerInterior from "../images/fortuner_back.jpg";
...
const masterFleetData = [
  { 
    ...
    isAc: true, 
    img: fortunerMain,
    description: "The Toyota Fortuner is the undisputed king of off-road luxury. Built for those who demand power and presence, it offers a commanding view of the road and robust 4x4 capability. The interior is a sanctuary of premium materials, featuring plush leather seating and state-of-the-art climate control. Whether navigating rugged terrains or city boulevards, the Fortuner delivers an elite travel experience."
  }
];

const FortunerDetails = () => {
  ...
  if (!carData) {
    return <div className="min-h-screen bg-black flex items-center justify-center font-bold text-white uppercase tracking-tighter">Initializing VIP SUV...</div>;
  }
  ...
  const details = {
    ...
    images: [carData.img, fortunerFront, fortunerInterior],
    features: [ `${carData.seats}`, "DUAL ZONE AC", "4X4 CAPABLE", `${carData.bags}` ]
  };

  const packagesData = [
    { id: 'city', title: 'LOCAL RIDE — 8HR 80KM', basePrice: 7500, extraKmRate: 35, extraHrRate: 500, hasExtras: true },
    { id: 'outstation', title: 'OUTSTATION — MIN 300KM / DAY', basePrice: 10500, extraKmRate: 40, extraHrRate: 0, hasExtras: true },
    { id: 'airport', title: 'AIRPORT VIP TRANSFER', basePrice: 3500, extraKmRate: 0, extraHrRate: 0, hasExtras: false }
  ];
  ...
  return (
    <div className="font-sans bg-black min-h-screen pt-24 md:pt-32 pb-20 px-4 sm:px-6 lg:px-8 relative selection:bg-gray-700 selection:text-white">
          ...
          <div className="flex items-center gap-4 md:gap-5">
            <div className="flex items-center text-white">
                <div className="w-12 h-12 md:w-14 md:h-14 rounded-full border-[1.5px] border-white flex items-center justify-center">
                  <span className="font-bold text-[10px] md:text-xs tracking-tighter text-white">4X4</span>
                </div>
            </div>
      ...
      {/* --- PACKAGE SELECTION (White Card Theme) --- */}
                ...
                <div className={`px-6 md:px-12 overflow-hidden transition-all duration-500 ${isActive ? 'max-h-[800px] pb-8 opacity-100' : 'max-h-0 opacity-0'}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-6 mt-4 bg-slate-50 p-6 rounded-2xl border border-slate-200">
                    <div className="flex flex-col">
                      <label className="text-slate-600 text-[10px] md:text-xs font-bold uppercase tracking-widest mb-2">Booking Date *</label>
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-5 mt-4 bg-[#181818] p-6 md:p-8 rounded-2xl shadow-lg border border-black">
                    <div>
                      <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-1">Estimated VIP Fare</p>
                      <p className="text-3xl md:text-4xl font-black text-white">₹ {currentTotal.toLocaleString('en-IN')}</p>
                      <p className="text-gray-500 text-[10px] mt-1 font-medium">Tolls & state taxes extra as per actuals</p>
                    </div>
                    <button 
...
export default FortunerDetails;