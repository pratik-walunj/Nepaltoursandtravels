...
// Assets - Replace these with your actual Vintage Luxury Car image paths
import vintageLuxuryMain from "../images/Vintage-Luxury-Car.jpg"; 
import vintageLuxuryInterior from "../images/Vintage-Luxury-Car-front.jpg";
import vintageLuxuryRear from "../images/Vintage-Luxury-Car-back.jpg";
...
const masterFleetData = [
  { 
    ...
    path: "vintage-luxury", 
    type: "Vintage Sedan", 
    models: "Vintage Luxury Car", 
    seats: "4 SEATS", 
    bags: "2 BAGS", 
    pricePerKm: 150, // Premium pricing for the luxury heritage fleet
    rating: 5.0, 
    isAc: true, 
    img: vintageLuxuryMain,
    description: "The Vintage Luxury Car is the crown jewel of our heritage fleet, offering an unparalleled blend of aristocratic elegance and classic automotive art. Perfectly maintained to preserve its original grandeur, this sedan features handcrafted wood accents, premium upholstery, and a smooth, stately ride. It is the definitive choice for high-end weddings, VIP arrivals, and exclusive period-themed tours where making a grand impression is the only priority."
  }
];

const VintageLuxuryDetails = () => {
  ...
  if (!carData) {
    return <div className="min-h-screen bg-black flex items-center justify-center font-bold text-white uppercase tracking-tighter">Polishing Chrome...</div>;
  }

  const details = {
    brand: "VINTAGE SEDAN",
    model: carData.models.toUpperCase(),
    category: "ELITE HERITAGE COLLECTION",
    driveType: "CHAUFFEUR DRIVEN",
    description: carData.description,
    images: [carData.img, vintageLuxuryInterior, vintageLuxuryRear],
    features: [ `${carData.seats}`, "LUXURY INTERIOR", "CLIMATE CONTROL", `${carData.bags}` ]
  };

  const packagesData = [
    { id: 'city', title: 'GRAND CITY TOUR — 8HR 80KM', basePrice: 35000, extraKmRate: 150, extraHrRate: 4000, hasExtras: true },
    { id: 'wedding', title: 'ROYAL WEDDING — 4HR 40KM', basePrice: 25000, extraKmRate: 200, extraHrRate: 5000, hasExtras: true },
    { id: 'airport', title: 'ELITE HERITAGE TRANSFER', basePrice: 12000, extraKmRate: 0, extraHrRate: 0, hasExtras: false }
  ];
    ...
    <div className="font-sans bg-black min-h-screen pt-24 md:pt-32 pb-20 px-4 sm:px-6 lg:px-8 relative selection:bg-gray-700 selection:text-white">
      ...
      <div className="w-full max-w-[1000px] mx-auto bg-[#181818] rounded-[2.5rem] md:rounded-[3rem] p-6 md:p-10 shadow-2xl mb-8 border border-white/5">
        ...
        <div className="flex flex-row items-center justify-between mb-8 px-1 md:px-2">
          <div className="flex items-center gap-4 md:gap-5">
            <div className="flex items-center text-white">
                <div className="w-12 h-12 md:w-14 md:h-14 rounded-full border-[1.5px] border-white flex items-center justify-center">
                  <span className="font-bold text-[9px] md:text-[10px] tracking-tighter text-white text-center leading-none">ROYAL</span>
                </div>
            </div>
          ...
          <div className="flex flex-col items-end">
            {/* <span className="text-amber-400 text-[10px] md:text-xs font-black uppercase tracking-[0.1em] bg-amber-500/10 px-3 py-1 rounded-md mb-1">
        ...
        {/* --- IMAGE CONTAINER (Light Gray Studio Theme) --- */}
      ...
      <div className="w-full max-w-[1000px] mx-auto mt-12">
        <h2 className="text-lg md:text-xl font-bold text-white tracking-widest uppercase mb-6 pl-2">
          Select Your Royal Package
        </h2>

        <div className="bg-white rounded-[2rem] overflow-hidden flex flex-col shadow-2xl border border-slate-200">
          {packagesData.map((pkg, index) => {
            ...
            return (
              <div key={pkg.id} className={`border-b border-slate-200 ${index === packagesData.length - 1 ? 'border-b-0' : ''}`}>
                <div 
                  className={`flex items-center justify-between p-6 md:p-8 cursor-pointer transition-colors ${isActive ? 'bg-slate-50' : 'hover:bg-slate-100'}`}
                  onClick={() => {
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`w-3.5 h-3.5 rounded-full border-2 transition-colors ${isActive ? 'bg-amber-600 border-amber-600' : 'border-slate-300'}`}>
                ...
                <div className={`px-6 md:px-12 overflow-hidden transition-all duration-500 ${isActive ? 'max-h-[800px] pb-8 opacity-100' : 'max-h-0 opacity-0'}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-6 mt-4 bg-slate-50 p-6 rounded-2xl border border-slate-200">
                    <div className="flex flex-col">
                      <label className="text-slate-600 text-[10px] md:text-xs font-bold uppercase tracking-widest mb-2">Event Date *</label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-8 px-2">
                      <div className="flex flex-col">
                        <label className="text-slate-600 text-[10px] font-bold uppercase mb-2 flex justify-between">
                          ...
                          <span className="text-black bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200">₹{pkg.extraKmRate}/KM</span>
                        </label>
                      ...
                      <div className="flex flex-col">
                        <label className="text-slate-600 text-[10px] font-bold uppercase mb-2 flex justify-between">
                          ...
                          <span className="text-black bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200">₹{pkg.extraHrRate}/HR</span>
                        </label>
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-5 mt-4 bg-[#181818] p-6 md:p-8 rounded-2xl shadow-lg border border-black">
                    <div>
                      <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-1">Estimated Journey Total</p>
                      <p className="text-3xl md:text-4xl font-black text-white">₹ {currentTotal.toLocaleString('en-IN')}</p>
                      <p className="text-gray-500 text-[10px] mt-1 font-medium">Tolls & specialty maintenance included</p>
                    </div>
                    <button 
                      onClick={(e) => handleProceedToPay(e, pkg, currentTotal)}
                      className="w-full md:w-auto bg-white hover:bg-slate-200 text-black font-black uppercase tracking-[0.15em] text-[10px] py-4 px-10 rounded-xl transition-all shadow-md hover:-translate-y-1 active:translate-y-0"
                    >
                      Reserve Royal Vintage
                    </button>
                  </div>
                </div>
              </div>
...
export default VintageLuxuryDetails;