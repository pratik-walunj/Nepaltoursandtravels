...
// Master Data with specific Audi A8 details and description
const masterFleetData = [
  { 
    ...
    isAc: true, 
    img: audia8,
    description: "Experience the pinnacle of automotive luxury with the Audi A8. Designed for those who demand the best, it offers a whisper-quiet cabin, premium plush leather seating, and state-of-the-art climate control. Featuring extra legroom, ambient lighting, and superior suspension, the Audi A8 delivers unparalleled elegance and an exceptionally smooth ride. Ideal for VIP airport transfers, corporate delegations, and high-profile events."
  }
];
...
const CabDetails = () => {
  ...
  // 1. Fetch Data UseEffect (Defaults to Audi A8 if no path is given)
  useEffect(() => {
    const foundCar = masterFleetData.find(car => car.path === carName) || masterFleetData.find(car => car.id === 99);
    setCarData(foundCar);
  }, [carName]);
  ...
  useEffect(() => {
    if (carData) {
      const interval = setInterval(() => {
        setActiveImage((prevIndex) => (prevIndex + 1) % 3); // 3 images total hain
  ...
  const details = {
    ...
    images: [
      carData.img,  
      audia6frontview, // Placeholder for A8 front
      audia6backview   // Placeholder for A8 back
    ],
    features: [
      `${carData.seats}`,
