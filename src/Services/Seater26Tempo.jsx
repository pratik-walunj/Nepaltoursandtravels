...
const masterFleetData = [
  { 
    id: 203, 
    path: "26-seater-tempo-traveller", 
    type: "Force", 
...
const CabDetails = () => {
  ...
  useEffect(() => {
    const foundCar = masterFleetData.find(car => car.path === carName) || masterFleetData.find(car => car.id === 203);
    setCarData(foundCar);
  }, [carName]);
  ...
  // Structuring data for 26-Seater Tempo Traveller
  const details = {
    brand: carData.type.toUpperCase(),
    model: carData.models.toUpperCase(),
    category: "MINI-BUS / MASSIVE GROUP", // Updated Category for 26-Seater
    driveType: "CHAUFFEUR DRIVEN",
    description: carData.description,
    images: [
      carData.img, // Side View
      seater26frontview, // Placeholder Front
      seater26backview  // Placeholder Back
    ],
    features: [
      `${carData.seats}`,
      carData.isAc ? "DUAL AC" : "NON-AC", // Dual AC highlight for heavy vehicles
      "DIESEL", 
      `${carData.bags}`
    ]
  };

  // Mock Packages Data (Pricing adjusted for 26-Seater Mini-Bus)
  const packagesData = [
    {
      id: 'city',
          ...
          <div className="flex items-center gap-4 md:gap-5">
            <div className="flex items-center text-white">
                ...
                <div className="w-12 h-12 md:w-14 md:h-14 rounded-full border-[1.5px] border-white flex items-center justify-center shadow-[0_0_15px_rgba(255,255,255,0.1)] relative">
                  <span className="font-bold text-xs md:text-sm tracking-widest text-white">BUS</span>
                </div>
            </div>
