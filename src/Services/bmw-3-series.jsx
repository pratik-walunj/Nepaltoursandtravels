...
import bmwFront from "../images/bmw_front.jpg"; 
import bmwInterior from "../images/bmw_back.jpg"; 
...
const masterFleetData = [
  { 
    ...
    isAc: true, 
    img: bmwMain,
    description: "The BMW 320d is the ultimate definition of a luxury sport sedan. Combining TwinPower Turbo diesel efficiency with legendary German engineering, it offers a ride that is as thrilling as it is comfortable. Featuring premium leather upholstery, a state-of-the-art iDrive infotainment system, and advanced safety features, it is the perfect choice for high-profile corporate travel, weddings, or making a grand entrance at any event."
  }
];
...
const CabDetails = () => {
  ...
  if (!carData) {
    return <div className="min-h-screen bg-black flex items-center justify-center font-bold text-white uppercase tracking-tighter">Igniting Engine...</div>;
  }

  const details = {
    brand: carData.type.toUpperCase(),
    model: carData.models.toUpperCase(),
    category: "ELITE SEDAN COLLECTION",
    driveType: "CHAUFFEUR DRIVEN",
    description: carData.description,
    images: [carData.img, bmwFront, bmwInterior],
    features: [ `${carData.seats}`, "CLIMATE CONTROL", "SUNROOF", `${carData.bags}` ]
  };

  const packagesData = [
    { id: 'city', title: 'LOCAL CORPORATE — 8HR 80KM', basePrice: 8500, extraKmRate: 45, extraHrRate: 800, hasExtras: true },
    { id: 'outstation', title: 'OUTSTATION — MIN 300KM / DAY', basePrice: 18000, extraKmRate: 45, extraHrRate: 1000, hasExtras: true },
    { id: 'airport', title: 'AIRPORT VIP TRANSFER', basePrice: 4500, extraKmRate: 0, extraHrRate: 0, hasExtras: false }
  ];
  ...
  return (
    <div className="font-sans bg-black min-h-screen pt-24 md:pt-32 pb-20 px-4 sm:px-6 lg:px-8 relative selection:bg-gray-700 selection:text-white">
      ...
      {/* --- 1. MAIN CAR DETAILS CARD (EXACT SAME AS TEMPO PAGE) --- */}
      ...
      {/* --- 2. EXPLORE OUR PACKAGES SECTION (White Background Style) --- */}
                ...
                <div className={`px-6 md:px-12 overflow-hidden transition-all duration-500 ${isActive ? 'max-h-[800px] pb-8 opacity-100' : 'max-h-0 opacity-0'}`}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-6 mt-4 bg-slate-50 p-6 rounded-2xl border border-slate-200">
                    <div className="flex flex-col">
                      <label className="text-slate-600 text-[10px] md:text-xs font-bold uppercase tracking-widest mb-2">Choose Date *</label>
                    ...
                    <div className="flex flex-col">
                      <label className="text-slate-600 text-[10px] md:text-xs font-bold uppercase tracking-widest mb-2">Choose Time *</label>
                  ...
                  {pkg.hasExtras && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-8 px-2">
                      <div className="flex flex-col">
                        ...
                        <input type="number" value={extraKms} onChange={(e) => setExtraKms(e.target.value)} placeholder="e.g. 50" className="bg-transparent border-b-2 border-slate-300 text-slate-800 pb-1.5 outline-none font-bold focus:border-black transition-colors" />
                      </div>
                      <div className="flex flex-col">
                        <label className="text-slate-600 text-[10px] font-bold uppercase mb-2 flex justify-between">
                          <span>Extra Hrs</span>
                        ...
                        <input type="number" value={extraHrs} onChange={(e) => setExtraHrs(e.target.value)} placeholder="e.g. 2" className="bg-transparent border-b-2 border-slate-300 text-slate-800 pb-1.5 outline-none font-bold focus:border-black transition-colors" />
                      </div>
                    </div>
                  )}
