...
import kyichu from "../images/kyichu.png"
...
import { 
  ...
  Camera, Bus, Award, Users, Mountain 
} from 'lucide-react';

// --- BHUTAN: PARO TAKTSANG PACKAGE DATA ---
const packageData = {
  ...
  images: [
    ParoTaktsang, // Tiger's Nest
    dzong, // Bhutan Dzong
    himalayan  // Himalayan Landscape
  ],
  itinerary: [
    {
      day: 1,
      title: "Arrival at Paro & Local Sightseeing",
      image: dzong,
      description: "Arrive at Paro International Airport. Our representative will meet you and transfer you to your hotel. Later, visit the Rinpung Dzong and the National Museum of Bhutan (Ta Dzong). Overnight stay in Paro."
    },
    {
      day: 2,
      title: "Hike to Taktsang Lhakhang (Tiger's Nest)",
      image: ParoTaktsang,
      description: "After breakfast, embark on a scenic hike to the iconic Tiger’s Nest monastery, perched on a cliff 900 meters above Paro Valley. Enjoy tea at the cafeteria with panoramic views. Evening at leisure. Overnight stay in Paro."
    },
    {
      day: 3,
      title: "Paro Valley Exploration",
      image: kyichu,
      description: "Visit the Drukgyel Dzong ruins and Kyichu Lhakhang, one of the oldest temples in Bhutan. Explore the local market in the evening to buy authentic Bhutanese crafts. Overnight stay in Paro."
    },
    {
      day: 4,
      title: "Departure from Paro",
      image: paroairport,
      description: "Transfer to Paro International Airport for your return flight."
    }
  ],
  highlights: [
    "4 Days intensive Bhutanese experience featuring the iconic Tiger's Nest hike.",
    "Comprehensive guided tours of Paro Valley and the National Museum.",
    "Sustainable Development Fee (SDF) assistance included.",
    "Private AC vehicle for all transfers and local sightseeing.",
    "Read More"
  ],
  transfer: [
    "Airport pickup and drop-off in Paro.",
    "All local transportation in private SUV/Coaster as per group size.",
    "Driver allowances, fuel, and permit fees included."
  ],
  sightseeing: [
    "Paro: Taktsang Lhakhang, Rinpung Dzong, Kyichu Lhakhang, Ta Dzong.",
    "Paro Town: Authentic local markets and traditional farms."
  ],
  accommodation: [
    "Paro: 3 Nights in a 3-star or 4-star boutique Bhutanese hotel."
  ],
  meals: [
    "Daily buffet breakfast at the hotel.",
    "Welcome traditional Bhutanese dinner.",
    "Complimentary bottled water during sightseeing."
  ],
  inclusionExclusions: {
    inclusions: [
      "Sustainable Development Fee (SDF) of $100 per night (for non-Indians).",
      "Bhutan Entry Visa/Permit fees.",
...
const vehicleData = [
  { name: "SUV (Hyundai Santa Fe/Toyota)", capacity: 4 },
  { name: "Hiace Van", capacity: 8 },
  ...
  return (
    <div className="font-sans bg-[#f8f9fa] min-h-screen pb-20">
      <div className="w-full h-[250px] md:h-[350px] lg:h-[400px] mb-6 relative">
        <img src={packageData.topBannerImage} alt="Paro Banner" className="w-full h-full object-cover" />
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          <div className="w-full lg:w-[68%]">
            <div className="flex flex-col md:flex-row gap-2 h-[300px] md:h-[400px] mb-6 relative rounded-lg overflow-hidden">
              <div className="absolute top-4 left-4 z-10 bg-white px-3 py-1 rounded text-orange-600 text-xs font-bold flex items-center shadow-md"><Mountain size={14} className="mr-1" /> Paro Region</div>
              <div className="w-full md:w-2/3 h-full overflow-hidden"><img src={packageData.images[0]} alt="Tiger's Nest" className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" /></div>
              <div className="hidden md:flex flex-col gap-2 w-full md:w-1/3 h-full">
                <div className="flex-1 overflow-hidden"><img src={packageData.images[1]} alt="Dzong" className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" /></div>
                <div className="flex-1 overflow-hidden relative cursor-pointer group"><img src={packageData.images[2]} alt="Bhutan" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" /></div>
              </div>
            </div>

            <div className="flex items-center gap-6 mb-10 pb-6 border-b border-gray-200 overflow-x-auto hide-scrollbar">
              {[{ icon: <BedDouble size={24} />, label: "Hotel", color: "bg-orange-500" }, { icon: <Utensils size={24} />, label: "Meals", color: "bg-yellow-500" }, { icon: <Camera size={24} />, label: "Sightseeing", color: "bg-blue-600" }, { icon: <Bus size={24} />, label: "Transfer", color: "bg-green-500" }, { icon: <Award size={24} />, label: "Highlights", color: "bg-red-400" }].map((item, i) => (
            ...
            {activeMainTab === 'calculate price' && (
              <div className="bg-white p-6 md:p-8 rounded-lg shadow-sm border border-gray-100 animate-fadeIn">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                  <div><label className="block text-sm font-semibold text-gray-700 mb-2">From <span className="text-red-500">*</span></label><select className="w-full border border-gray-300 rounded px-3 py-2.5 text-sm text-gray-700 outline-none focus:border-red-400 bg-white"><option>Paro Airport</option><option>Phuentsholing Border</option><option>Delhi (Excl. Flight)</option></select></div>
            ...
            <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6 text-center">
              ...
              <div className="bg-[#f9f9f9] border border-gray-100 rounded-md py-3 px-4 flex items-center justify-center mb-5">
                <div className="text-left flex items-center text-xs text-gray-600">
                  ...
                  <span><strong className="text-gray-800 text-sm">Earn +1200</strong><br/>Holiday Reward points</span>
            ...
            <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
              ...
              <div className="space-y-3">
                <div className="border border-dashed border-gray-300 rounded bg-[#fcfcfc] p-3 opacity-60">
                  <div className="flex items-center text-sm font-semibold text-gray-600 mb-1"><input type="radio" disabled className="mr-2" /> BHUTAN2026</div>
                  <p className="text-[11px] text-gray-500 ml-6">Upto 10K off on Himalayan Tours*</p>
                </div>
              </div>
            </div>
          </div>
...
export default ParoTaktsangTour;